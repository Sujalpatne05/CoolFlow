...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("rounded-2xl border-2 border-border bg-card text-card-foreground shadow-soft hover:shadow-elevated transition-shadow duration-300", className)}
      {...props}
    />
  ),
);
