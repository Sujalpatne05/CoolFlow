...
function getStatusBadge(status: TicketStatus) {
  ...
  if (status === "in-progress") return "bg-secondary-50 text-secondary-700 border-secondary-200";
  return "bg-primary-50 text-primary-700 border-primary-200";
}

function getPriorityBadge(priority: TicketPriority) {
  if (priority === "high") return "bg-red-100 text-red-700";
  if (priority === "medium") return "bg-secondary-100 text-secondary-800";
...
export default function SuperAdminSupport() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { label: "In Progress", value: inProgress, icon: <Clock className="h-6 w-6 text-secondary-600" />, color: "bg-secondary-50 border-secondary-100", text: "text-secondary-700" },
      { label: "Resolved", value: resolved, icon: <CheckCircle className="h-6 w-6 text-primary-600" />, color: "bg-primary-50 border-primary-100", text: "text-primary-700" },
    ];
  }, [ticketList]);
              ...
              <tbody>
                {filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${(ticket.sla || "").includes("left") ? "bg-secondary-100 text-secondary-800" : "bg-primary-100 text-primary-700"}`}>
